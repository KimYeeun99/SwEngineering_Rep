    ...
    fetch(fetchUrl).then(function(response) {
        if (response.status === 200 || response.status === 201) {
            ...
            response.text().then(function(text) {
                ...
                info.innerHTML += '<div style="font-size: 20px; font-weight:bolder; margin: 10px 0;">'+'단체동 캠핑장'+'</div>'
                info.innerHTML +='<div style="font-size: 16px; margin-bottom: 5px;">랜덤배정</div>'
                info.innerHTML +='<div style="font-size: 16px; margin-bottom: 10px;">'+'</div>'
...
getArticle(fetchUrl);
function go_url(){
    location.href = 'http://localhost:8085/room/reservation';
}